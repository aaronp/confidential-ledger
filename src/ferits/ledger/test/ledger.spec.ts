...
import { getPublicSummary } from "../core/ledger.verify";
...
describe("Confidential Ledger", () => {
  ...
  describe("Public Summary", () => {
    it("provides correct public summary", async () => {
      const allocations: MintAllocation[] = [
        { holder: alice, amount: 100 },
        { holder: bob, amount: 200 },
        { holder: carol, amount: 300 },
      ...
      const summary = getPublicSummary(ledger);

      expect(summary.version).toBe("1");
      expect(summary.participantCount).toBe(3);
      expect(summary.total.T).toBe("600");
      expect(summary.verified).toBe(true);
    });

    it("marks tampered ledger as unverified in summary", async () => {
      const allocations: MintAllocation[] = [{ holder: alice, amount: 100 }];
      const ledger = await mintPoC(allocations);
