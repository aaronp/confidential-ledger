...
import type { KeyPair, Ledger, Entry, SerializedKeyPair } from "../lib/ledger";
import { mintPoC, verifyTotal, getUserView, deserializeKeyPair, updateOwnEntry } from "../lib/ledger";
import { Button } from "./ui/button";
import { Input } from "./ui/input";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "./ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, TableFooter } from "./ui/table";
import { Badge } from "./ui/badge";
import { Plus, FileSpreadsheet, CheckCircle2, XCircle, Eye, EyeOff, Edit3, Save, X, RotateCcw, HelpCircle, ChevronDown, ChevronUp, AlertTriangle } from "lucide-react";
...
export function LedgerSpreadsheet({ currentIdentity }: LedgerSpreadsheetProps) {
  ...
  const [originalLedger, setOriginalLedger] = useState<Ledger | null>(null);
  ...
  const handleAllocationChange = (index: number, field: keyof AllocationInput, value: string) => {
  ...
  const handleMintLedger = async () => {
    ...
    const validAllocations = allocations.filter(
      a => a.holderId.trim() !== "" && a.amount.trim() !== "" && !isNaN(Number(a.amount))
    );
    ...
    const mintInputs = validAllocations.map(a => {
      ...
      if (!identity) {
        throw new Error(`Identity "${a.holderId}" not found. Create it first.`);
      }
      return {
  ...
  const handleStartEdit = (index: number, entry: Entry) => {
  ...
  const hasAnyModifications = (): boolean => {
    ...
    return ledger.entries.some((entry, idx) => {
      ...
      return entry.commit !== original.commit || entry.openingEncrypted !== original.openingEncrypted;
    });
  };
  ...
  const handleSaveBalanceEdit = async () => {
    ...
    try {
      const updatedLedger = await updateOwnEntry(ledger, currentIdentity, newAmount);
      setLedger(updatedLedger);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Create a ledger with Pedersen commitments. Each entry's value is encrypted and only visible to the holder.
        </CardDescription>
        ...
        <div className="mt-4">
          ...
          {showHelp && (
            <div className="mt-3 p-4 bg-muted/50 rounded-lg text-sm space-y-3">
              <div>
                <h4 className="font-semibold mb-1">Understanding the Ledger Components</h4>
              ...
              <div className="space-y-2 pl-4 border-l-2 border-primary/20">
                <div>
                  ...
                  <p className="text-muted-foreground">The actual balance amount (e.g., 100 tokens)</p>
                </div>

                <div>
                  <span className="font-semibold text-primary">Randomness (r):</span>
                  <p className="text-muted-foreground">A secret random number used to hide the value</p>
                </div>

                <div>
                  <span className="font-semibold text-primary">Commitment (C):</span>
                  <p className="text-muted-foreground">
                    A cryptographic hash: <code className="bg-background px-1 py-0.5 rounded">C = G×v + H×r</code>
                    <br />
                ...
                <div>
                  <span className="font-semibold text-primary">Encrypted Opening:</span>
                  <p className="text-muted-foreground">
                    The values {"{v, r}"} encrypted so <strong>only the holder</strong> can read them
                  </p>
                </div>
              </div>
              ...
              <div className="pt-2 border-t border-border/50">
                ...
                <ul className="list-disc list-inside text-muted-foreground space-y-1">
                  <li>You can decrypt your own encrypted opening to see your balance</li>
                  <li>You can verify the decrypted value matches the commitment</li>
                  <li>Others see only the commitment (which hides your balance)</li>
                  <li>Everyone can verify the total without seeing individual balances</li>
                </ul>
              </div>
              ...
              <div className="pt-2 border-t border-border/50">
                ...
                <p className="text-muted-foreground">
                  If someone edits the commitment or encrypted opening, the verification fails and a
                  <AlertTriangle className="w-3 h-3 inline mx-1 text-red-600" />
                  warning appears. The value might still decrypt, but it won't match the commitment!
                </p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
              ...
              {allocations.map((alloc, index) => (
                <div key={index} className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) => handleAllocationChange(index, "holderId", e.target.value)}
            ...
            <div className="flex items-center gap-4 pt-2">
              <label className="flex items-center gap-2 text-sm">
                ...
                />
                <span className="text-foreground">Allow users to update their own balance</span>
              </label>
            </div>
          ...
          <>
            ...
            {editMode && (
              <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm">
                ...
                <p className="text-yellow-800">
                  You can now edit commitments and encrypted openings to simulate data corruption.
                  Watch how verification fails and balances become unreadable when data is tampered!
                </p>
              </div>
            )}
            ...
            <Table>
              ...
              <TableBody>
                {ledger.entries.map((entry, index) => {
                  ...
                  const commitModified = originalLedger && entry.commit !== originalLedger.entries[index].commit;
                  const encryptedModified = originalLedger && entry.openingEncrypted !== originalLedger.entries[index].openingEncrypted;
                  ...
                  return (
                    <TableRow key={index} className={isMyEntry ? "bg-primary/5" : ""}>
                      ...
                      <TableCell className={`font-mono text-xs ${commitModified ? "bg-orange-100 border-2 border-orange-400 dark:bg-orange-900/30 dark:border-orange-600" : ""}`}>
                        {isEditing ? (
                          <Input
                            value={editedCommit}
                          ...
                          <span className="break-all">{entry.commit.slice(0, 16)}...</span>
                        )}
                      </TableCell>
                      <TableCell className={`font-mono text-xs ${encryptedModified ? "bg-orange-100 border-2 border-orange-400 dark:bg-orange-900/30 dark:border-orange-600" : ""}`}>
                        {isEditing ? (
                          <Input
                          ...
                          <span className="break-all">{entry.openingEncrypted.slice(0, 16)}...</span>
                        )}
                      </TableCell>
                      <TableCell className="text-right">
                        {myBalance !== null ? (
                          <div className="flex items-center justify-end gap-2">
                            <span className={`font-semibold ${userView.myEntryValid ? "text-green-600" : "text-red-600"}`}>
                              {myBalance.toString()}
                            </span>
              ...
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell className="text-right font-semibold">
                    {userView?.publicTotal !== null && userView?.publicTotal !== undefined
            ...
            {currentIdentity && userView && (
              <div className="text-sm space-y-3 p-4 bg-muted rounded-lg">
                <h4 className="font-semibold">Your View as {currentIdentity.id}</h4>
                <div className="grid grid-cols-2 gap-2">
                    ...
                    {isEditingMyBalance ? (
                      <div className="flex items-center gap-2 mt-1">
                        <Input
                          ...
                          autoFocus
                        />
                        <Button onClick={handleSaveBalanceEdit} size="sm" className="h-7 px-2">
                          <Save className="w-3 h-3" />
                        </Button>
                        <Button onClick={handleCancelBalanceEdit} size="sm" variant="outline" className="h-7 px-2">
                          <X className="w-3 h-3" />
                        </Button>
                      </div>
                    ) : (
                      <span className="font-medium">
                        {userView.myBalance !== null ? userView.myBalance.toString() : "Not in ledger"}
                  ...
                  <div>
                    ...
                    <span className="font-medium">
                      {userView.publicTotal !== null ? userView.publicTotal.toString() : "Unverified"}
                    </span>
                  </div>
                </div>
                <div>
